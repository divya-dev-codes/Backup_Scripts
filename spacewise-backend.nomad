job "spacewise-backend" {
  datacenters = ["dc1"]
  type = "service"

  group "spacewise-backend"{
    count = 1
    spread {
  ...
  task "spacewise-backend" {
    ...
    config {
      ...
      volumes = [
        "local/nomad.env:/spacewise-backend/devops/nomad.env"
      ]
    }
    ...
    template {
        ...
        change_mode = "noop"
        data        = <<EOF
{{- with nomadVar "nomad/jobs/spacewise-backend" -}}
